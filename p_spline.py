...
imgplot = matplotlib.pyplot.imshow(Y[0,:].reshape(640,480),cmap = cm.Greys_r)
...
matplotlib.pyplot.title(r'$\lambda = 0.01$, # knots = 200', fontsize=15)
matplotlib.pyplot.show()
...
#eigenvector_matrix = numpy.hstack((eigen_pairs[0][1].reshape(200,1),eigen_pairs[1][1].reshape(200,1),eigen_pairs[2][1].reshape(200,1),eigen_pairs[3][1].reshape(200,1)))
...
#Now visualise the univariate spline regression using T_cropped as spline coefficients
imgplot = matplotlib.pyplot.imshow(T_cropped.reshape(640,480),cmap = cm.Greys_r)
matplotlib.pyplot.show()
...
#Now visualise the discretized T_cropped
imgplot = matplotlib.pyplot.imshow(T_cropped[:,0].reshape(640,480),cmap = cm.Greys_r)
matplotlib.pyplot.show() 
...
##########################################################################
#####             discrete inference output to continuous T_cropped
##########################################################################


##########################################################################
#####             Reverse principal component analysis
##########################################################################
coefficients_pixels_hat = T_cropped.dot(eigenvector_matrix.T)
