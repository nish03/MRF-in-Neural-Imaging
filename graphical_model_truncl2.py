...
#####threshold is the mean of the 1d array in each iteration
########################################################################### 
...
for i in range(numVar):
    for l in range(numLabels):
	    threshold = numpy.repeat(numpy.mean( (Y[:,i] - basis.dot(a_offset[:,i,l]))**2 ) ,1066)
	    unary_energy[i,l] = sqrt(sum(numpy.minimum((Y[:,i] - basis.dot(a_offset[:,i,l]))**2, threshold)))
...
for x in range(dimx):
    for y in range(dimy):
        ...
        if (x + 1 < dimx):
            for l in range(numLabels):
                for k in range(numLabels):
                    threshold = numpy.repeat(numpy.mean((a_offset[:,x,y,l] - a_offset[:,x+1,y,k])**2) ,no_of_splines)
                    pairwise_energy[l,k] = sqrt(sum(numpy.minimum((a_offset[:,x,y,l] - a_offset[:,x+1,y,k])**2, threshold )))
        ...
        if (y + 1 < dimy):
            for l in range(numLabels):
                for k in range(numLabels):
                    threshold = numpy.repeat(numpy.mean((a_offset[:,x,y,l] - a_offset[:,x,y+1,k])**2) ,no_of_splines)
                    pairwise_energy[l,k] = sqrt(sum(numpy.minimum((a_offset[:,x,y,l] - a_offset[:,x,y+1,k])**2, threshold )))
            pair_id = gm.addFunction(pairwise_energy)	
            variableIndex0 = y + x * dimy
