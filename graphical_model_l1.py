...
sigma = numpy.full((no_of_splines,),0.001) #offset
...
for i in range(numVar):
    for l in range(numLabels):
	    unary_energy[i,l] = sum(abs(Y[:,i] - basis.dot(a_offset[:,i,l]))) 
...
for x in range(dimx):
    for y in range(dimy):
        ...
        if (x + 1 < dimx):
            for l in range(numLabels):
                for k in range(numLabels):
                    pairwise_energy[l,k] = sum(abs(a_offset[:,x,y,l] - a_offset[:,x+1,y,k]))
        ...
        if (y + 1 < dimy):
            for l in range(numLabels):
                for k in range(numLabels):
                    pairwise_energy[l,k] = sum(abs(a_offset[:,x,y,l] - a_offset[:,x,y+1,k]))
            pair_id = gm.addFunction(pairwise_energy)	
            variableIndex0 = y + x * dimy
