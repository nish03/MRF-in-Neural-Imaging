...
def semiparamRegressionRaw(S2, X, B, P):
...
def semiparamRegression(S2, X, B, B2, P, P2, noPixels,num_clusters, num_knots):
    """Apply semiparametric regression framework to imaging data.
    S: m x n data cube with m time series of length n
    X: length m vector of discretized parametric function
    B: non parametric basis
    ...
    for i in range(0,len(lambdas)):
        ...
        beta_mrf = pm.pixel_mrf_model(num_knots,num_clusters,beta_mrf,S2,B2,noPixels) #beta_mrf_after_regularization
